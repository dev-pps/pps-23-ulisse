...
class TimesTest extends AnyWordSpec with Matchers:
  "ClockTime" should:
    "validate non in range values of hours and minutes" in:
      ClockTime(h = -12, m = 30) should be(Left(InvalidHours(Time(-12, 30))))
      ClockTime(h = 30, m = 30) should be(Left(InvalidHours(Time(30, 30))))
      ClockTime(h = 10, m = -30) should be(Left(InvalidMinutes(Time(10, -30))))
      ClockTime(h = 10, m = 60) should be(Left(InvalidMinutes(Time(10, 60))))
    ...
    "return user clock time when hours and minutes values are valid using withDefault method" in:
      ...
      clockTime.asTime shouldBe Time(validHours, validMinutes)
    ...
    "returns a default time that depends on context strategy" in:
      ...
      given DefaultTimeStrategy = t => Time(t.h % 24, t.m % 60)
      val defaultClockTime      = invalidClockTime.getOrDefault
      defaultClockTime.asTime shouldBe Time(2, 10)
//
