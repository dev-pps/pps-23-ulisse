...
import ulisse.entities.{Coordinate, Routes}
...
import ulisse.entities.timetable.Timetables.at
...
class TimetableTest extends AnyFlatSpec:
  ...
  private val stationE = Station("Station E", Coordinate(55, 0), 1) // 1 min from D

  object TestingRoutes:
    val routeA_B = Routes.Route(departure = stationA, arrival = stationB, TypeRoute.AV, railsCount = 2, length = 10)
    val routeB_C = Routes.Route(departure = stationB, arrival = stationC, TypeRoute.AV, railsCount = 2, length = 15)
    val routeC_D = Routes.Route(departure = stationC, arrival = stationD, TypeRoute.AV, railsCount = 2, length = 25)
    val routeD_E = Routes.Route(departure = stationD, arrival = stationE, TypeRoute.AV, railsCount = 2, length = 5)
...
//    val invalidTime = h(50).m(0)
////    val invalidTimetable: Either[ClockTimeErrors, PartialTimetable] =
////      train AV1000Train at invalidTime travelsOn routeA_B
////      PartialTimetable(train = AV1000Train, travelsOn = routeA_B, invalidDepartureTime)
//    for
//      route <- TestingRoutes.routeA_B
//    yield AV1000Train at invalidTime travelsOn route should be(Left(InvalidHours()))

  "Timetable" should "be built giving sequence of routes travelled by train that depart at some valid time" in:
    ...
    for
      ...
      D_E  <- routeD_E
      time <- h(9).m(30)
    yield AV1000Train at time travelsOn A_B travelsOn B_C travelsOn C_D travelsOn D_E asFinalRoute
