...
object TrainEditor:
  // DOMAIN
  val items: List[Train] = List(
    Trains.makeRegionalTrain("4044", 4, 400),
    Trains.makeRegionalTrain("3030", 2, 500),
    Trains.makeHighSpeedTrain("F-1000", 12, 900)
  )
  private val trainTypes = Trains.TrainType.values

  enum CarriageType:
    case Passenger
    case Goods

  val carriageTypes = CarriageType.values
