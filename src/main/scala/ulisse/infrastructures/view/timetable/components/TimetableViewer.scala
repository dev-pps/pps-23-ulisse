...
trait TimetableViewer extends Component:
  def update(data: List[TimetableEntry]): Unit

object TimetableViewer:
  def apply(timeEntries: List[TimetableEntry]): TimetableViewer =
    TimetableListView(timeEntries)

  private final case class TimetableListView(timeEntries: List[TimetableEntry]) extends ListView(timeEntries)
      with TimetableViewer:
