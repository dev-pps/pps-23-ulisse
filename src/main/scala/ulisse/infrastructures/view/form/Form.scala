...
object Form:
  ...
  case class ScheduleForm() extends Form:
    private val trainCombo         = JComponent.createInfoTextField("Field")
    private val station            = JComponent.createInfoTextField("Arriving/departure station")
    private val waitMinutesSpinner = JComponent.createInfoTextField("Waiting minutes")

    private val form = BaseForm("Timetable creation", trainCombo, station, waitMinutesSpinner)

    private val insertButton = JItem.button("Insert", trueButtonStyler)
    private val undoButton   = JItem.button("undo", falseButtonStyler)

    buttonPanel.contents += insertButton
    buttonPanel.contents += undoButton
