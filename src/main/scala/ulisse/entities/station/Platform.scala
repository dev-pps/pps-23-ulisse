...
object Platform:
  ...
  def apply(platformNumber: Int): Platform = PlatformImpl(math.max(1, platformNumber), None, None)
  ...
  private final case class PlatformImpl(
      ...
      currentDirection: Option[TrainAgentsDirection]
  ) extends Platform:
    // TODO evaluate if could be nice to introduce a control to check if the train is entering the station or is already moved
    override def putTrain(train: TrainAgent, direction: TrainAgentsDirection): Option[Platform] =
