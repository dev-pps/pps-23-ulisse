...
object TechnologyManagers:
  ...
  /** Companion object of trait [[TechErrors]] */
  object TechErrors:
  ...
  object TechnologyManager:
    ...
    private case class TechnologyManagerImpl[T <: Technology](technologies: Map[String, T])
      ...
      override def add(technology: T): Either[TechErrors, TechnologyManager[T]] =
        for
      ...
      override def technologiesList: List[T] = technologies.values.toList

      override def remove(name: String): Either[TechErrors, TechnologyManager[T]] =
        for
          t <- getBy(name)
        yield TechnologyManager(technologies.removed(t.name).values.toList)

      override def getBy(name: String): Either[TechErrors, T] =
